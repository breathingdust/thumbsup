...
func (c *CoreServiceReactionsCommand) Run(args []string) int {
	...
	for _, s := range coreServices {
		results = append(results, goGithubClient.GetIssuesForLabel(ctx, s)...)
	}
	...
	for i := 0; i < 20; i++ {
		...
		fmt.Printf("Title: %s, Url: %s, Reactions: %d, +1: %d, -1: %d, Hooray: %d, Heart: %d, Rocket: %d, Eyes: %d, Confused: %d \n",
			issue.GetTitle(), issue.GetURL(), *issue.Reactions.TotalCount, *issue.Reactions.PlusOne, *issue.Reactions.MinusOne, *issue.Reactions.Hooray, *issue.Reactions.Heart, *issue.Reactions.Rocket, *issue.Reactions.Eyes, *issue.Reactions.Confused)
	}
	return 0
}
